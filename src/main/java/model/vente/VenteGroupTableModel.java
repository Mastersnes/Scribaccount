...
public class VenteGroupTableModel extends AbstractTableModel {
	private Map<String, VenteGroup> model;
	private final VentePanel panel;

	public VenteGroupTableModel(final VentePanel panel) {
		super();
		this.panel = panel;
	}
    ...
    /**
     * Renvoi le groupe a la ligne indiquée
     * 
     * @param index
     * @return
     */
    public VenteGroup getGroupAt(final int index) {
        final VenteGroup item = this.model.values().toArray(new VenteGroup[model.size()])[index];
        return item;
    }

    public void remove(final VenteGroup venteGroup) {
        final String groupKey = venteGroup.getIdLivre() + Phrases.TRANSITION_LIVRE_PRESTA
                + venteGroup.getIdPresta();
        this.model.remove(groupKey);
        BDD.getInstance().save();
        fireTableDataChanged();
        panel.refreshTotal();
    }
